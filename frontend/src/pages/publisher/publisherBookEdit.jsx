...
const PublisherBookEdit = () =>{
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                // Lấy thông tin sách
                const bookResponse = await api.get(`/book/${id}`, {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const response = await api.put(`publisher/books/edit/${id}`, formData, {
                headers: { Authorization: `Bearer ${token}` },
            });

            if (response.status === 200) {
                navigate(`/publisher/books`); // Điều hướng về trang quản lý sách
            }
        } catch (error) {
...
export default PublisherBookEdit;